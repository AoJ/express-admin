...
Database.prototype.connect = function (user, pass, cb) {
    this.connection = mysql.createConnection({
        host: "lisa.allin1.cz", user: user, password: pass, typeCast: false, multipleStatements: true
    });
    ...
    if (err instanceof Error) {
        switch (err.code) {
            case 'ER_ACCESS_DENIED_ERROR':
                this.message = 'Non existing user or misspelled username or password';
                break;
